...
port number). The address is 127.0.0.1:8000
2. Is it UDP or TCP? How do you know?
  It is UDP  as it uses streams
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? It gets the data from the terminal (i.e
STDIN). You can tell by the read() call in the while loop.
4. How does the client program end? How can you tell that in the code?
  The client ends when the user doesn't input anything since it checks that
num_read > 1
*/
...
void *handle_client(void *arg) {
  ...
  char buf[64];
  int num_read;
  while ((num_read = read(client->cfd, buf, 64)) > 0) {
