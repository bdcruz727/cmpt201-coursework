...
int main() {
  ...
  FILE *fptr;
  fptr = fopen("public_key.pem", "r");
  pubkey = PEM_read_PUBKEY(fptr, NULL, NULL, NULL);
  if (pubkey == NULL) {
    handle_error("Error with PEM_read_PUBKEY");
  }
...
#define MAX_FILE_SIZE 512
  ...
  FILE *messagefp = fopen(message_path, "r");
  FILE *signfp = fopen(sign_path, "r");
  size_t msg_len = fread(message, 1, MAX_FILE_SIZE, messagefp);
  size_t sig_len = fread(signature, 1, MAX_FILE_SIZE, signfp);
  ...
  if (mdctx == NULL) {
    handle_error("mdctx create failed");
  }

  EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pubkey);

  EVP_DigestVerifyUpdate(mdctx, message, msg_len);
