...
1. How is the client sending data to the server? What protocol?
  It is using TCP indicated by SOCK_STREAM
2. What data is the client sending to the server?
  It is sending the strings in messages i.e "Hello"
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
    The argument passed is a struct that contains a run flag, a shared list and
a lock for pthread usage
2. How are received messages stored?
    They are stored as nodes in a linked list
3. What does `main()` do with the received messages?
    It prints out each message received
4. How are threads used in this sample code?
    There is a singular acceptor thread that accepts clients and creates client
threads. There is a client thread for each client that reads messages from one
client and appends to LL.


Explain the use of non-blocking sockets in this lab.
How are sockets made non-blocking?
  By using the O_NONBLOCK flag
What sockets are made non-blocking?
  The SFD and each CFD so that accept() and read() respectively does not block
Why are these sockets made non-blocking? What purpose does it serve?
  By making these sockets non blocking it prevents deadlocks and allows each thread to
  retry if there is no data
*/
